...
trimer = 'C:/Users/User/SEProm-master/train_data/TSS_Seq/Synechocystis sp. PCC6803'
paramVals = 'C:/Users/User/SEProm-master/train_data/new_trimer.xlsx'
trimer_cds = 'C:/Users/User/SEProm-master/train_data/CDS_Seq/Synechocystis_sp_PCC6803_CDS'

strInc = []
strDec = []

def dataCleaning(df):
    # df = df.drop(['trimer'], axis =1)
    duplicates = df.duplicated(subset = ['a','b','c','d','e','f','g','h','i','j','k','l','m','n','o','p','q','r','s','t','u'], keep = False)
    df2 = df[~duplicates]
    df1 = df[duplicates]
    df1 = (df1.groupby(df1.columns.tolist())
           .apply(lambda x: tuple(x.index))
           .reset_index(name='Trimer'))
    df1=df1.set_index('Trimer')
...
# to continue from here
    ...
    for motif in list_motifs:
        if motif in paramValues.columns:
            ...
            param_map['s'].append(paramValues[motif]['s'])
            param_map['t'].append(paramValues[motif]['t'])
            param_map['u'].append(paramValues[motif]['u'])
            ...
            for j in range(len(paramValues.columns)):
                if motif in paramValues.columns[j]:
                    ...
                    param_map['s'].append(paramValues[paramValues.columns[j]]['s'])
                    param_map['t'].append(paramValues[paramValues.columns[j]]['t'])
                    param_map['u'].append(paramValues[paramValues.columns[j]]['u'])
...
#main
sequence_map_tss = readSequenceFile.readSequenceFile(trimer)
sequence_map_cds = readSequenceFile.readSequenceFile(trimer_cds)
tri_seq_map = iterateSequences(sequence_map_tss)
#tri_seq_map.to_csv('tri_seq_map.csv')
tri_cds_seq_map = iterateSequences(sequence_map_cds)
#tri_cds_seq_map.to_csv('tri_cds_seq_map.csv')



#plotting to check if at 500, there is a dip CORRESPONDING TO EACH PARAMETRE(to indicate TSS)
a = [0 for i in range(975)]
b = [0 for i in range(975)]
c = [0 for i in range(975)]
d = [0 for i in range(975)]
e = [0 for i in range(975)]
f = [0 for i in range(975)]
g = [0 for i in range(975)]
h = [0 for i in range(975)]
i = [0 for z in range(975)]
j = [0 for i in range(975)]
k = [0 for i in range(975)]
l = [0 for i in range(975)]
m = [0 for i in range(975)]
n = [0 for i in range(975)]
o = [0 for i in range(975)]
p = [0 for i in range(975)]
q = [0 for i in range(975)]
r = [0 for i in range(975)]
s = [0 for i in range(975)]
t = [0 for i in range(975)]
u = [0 for i in range(975)]


a_cds = [0 for i in range(975)]
b_cds = [0 for i in range(975)]
c_cds = [0 for i in range(975)]
d_cds = [0 for i in range(975)]
e_cds = [0 for i in range(975)]
f_cds = [0 for i in range(975)]
g_cds = [0 for i in range(975)]
h_cds = [0 for i in range(975)]
i_cds = [0 for z in range(975)]
j_cds = [0 for i in range(975)]
k_cds = [0 for i in range(975)]
l_cds = [0 for i in range(975)]
m_cds = [0 for i in range(975)]
n_cds = [0 for i in range(975)]
o_cds = [0 for i in range(975)]
p_cds = [0 for i in range(975)]
q_cds = [0 for i in range(975)]
r_cds = [0 for i in range(975)]
s_cds = [0 for i in range(975)]
t_cds = [0 for i in range(975)]
u_cds = [0 for i in range(975)]


for z in range(len(sequence_map_tss)):
    a = np.add(a, tri_seq_map['normalized_params_map'][z]['a'])

    b = np.add(b, tri_seq_map['normalized_params_map'][z]['b'])

    c = np.add(c, tri_seq_map['normalized_params_map'][z]['c'])

    d = np.add(d, tri_seq_map['normalized_params_map'][z]['d'])

    e = np.add(e, tri_seq_map['normalized_params_map'][z]['e'])

    f = np.add(f, tri_seq_map['normalized_params_map'][z]['f'])

    g = np.add(g, tri_seq_map['normalized_params_map'][z]['g'])

    h = np.add(h, tri_seq_map['normalized_params_map'][z]['h'])

    i = np.add(i, tri_seq_map['normalized_params_map'][z]['i'])

    j = np.add(j, tri_seq_map['normalized_params_map'][z]['j'])

    k = np.add(k, tri_seq_map['normalized_params_map'][z]['k'])

    l = np.add(l,tri_seq_map['normalized_params_map'][z]['l'])

    m = np.add(m,tri_seq_map['normalized_params_map'][z]['m'])

    n = np.add(n,tri_seq_map['normalized_params_map'][z]['n'])

    o = np.add(o,tri_seq_map['normalized_params_map'][z]['o'])

    p = np.add(p, tri_seq_map['normalized_params_map'][z]['p'])

    q = np.add(q, tri_seq_map['normalized_params_map'][z]['q'])

    r = np.add(r, tri_seq_map['normalized_params_map'][z]['r'])

    s = np.add(s, tri_seq_map['normalized_params_map'][z]['s'])

    t = np.add(t, tri_seq_map['normalized_params_map'][z]['t'])

    u = np.add(u, tri_seq_map['normalized_params_map'][z]['u'])



for z in range(len(sequence_map_cds)):
    a_cds = np.add(a_cds, tri_cds_seq_map['normalized_params_map'][z]['a'])
    b_cds = np.add(b_cds, tri_cds_seq_map['normalized_params_map'][z]['b'])
    c_cds = np.add(c_cds, tri_cds_seq_map['normalized_params_map'][z]['c'])
    d_cds = np.add(d_cds, tri_cds_seq_map['normalized_params_map'][z]['d'])
    e_cds = np.add(e_cds, tri_cds_seq_map['normalized_params_map'][z]['e'])
    f_cds = np.add(f_cds, tri_cds_seq_map['normalized_params_map'][z]['f'])
    g_cds = np.add(g_cds, tri_cds_seq_map['normalized_params_map'][z]['g'])
    h_cds = np.add(h_cds, tri_cds_seq_map['normalized_params_map'][z]['h'])
    i_cds = np.add(i_cds, tri_cds_seq_map['normalized_params_map'][z]['i'])
    j_cds = np.add(j_cds, tri_cds_seq_map['normalized_params_map'][z]['j'])
    k_cds = np.add(k_cds, tri_cds_seq_map['normalized_params_map'][z]['k'])
    l_cds = np.add(l_cds,tri_cds_seq_map['normalized_params_map'][z]['l'])
    m_cds = np.add(m_cds,tri_cds_seq_map['normalized_params_map'][z]['m'])
    n_cds = np.add(n_cds,tri_cds_seq_map['normalized_params_map'][z]['n'])
    o_cds = np.add(o_cds,tri_cds_seq_map['normalized_params_map'][z]['o'])
    p_cds = np.add(p_cds,tri_cds_seq_map['normalized_params_map'][z]['p'])
    q_cds = np.add(q_cds,tri_cds_seq_map['normalized_params_map'][z]['q'])
    r_cds = np.add(r_cds,tri_cds_seq_map['normalized_params_map'][z]['r'])
    s_cds = np.add(s_cds,tri_cds_seq_map['normalized_params_map'][z]['s'])
    t_cds = np.add(t_cds,tri_cds_seq_map['normalized_params_map'][z]['t'])
    u_cds = np.add(u_cds,tri_cds_seq_map['normalized_params_map'][z]['u'])
...
s = [z/len(sequence_map_tss) for z in s]
t = [z/len(sequence_map_tss) for z in t]
u = [z/len(sequence_map_tss) for z in u]
...
s_cds = [z/len(sequence_map_cds) for z in s_cds]
t_cds = [z/len(sequence_map_cds) for z in t_cds]
u_cds = [z/len(sequence_map_cds) for z in u_cds]
...
tss_df.to_csv("tss_df_syne_tri.csv")
cds_df.to_csv("cds_df_syne_tri.csv")
...
axs[17].plot(r_cds)
axs[18].plot(s,label = 's')
axs[18].plot(s_cds)
axs[19].plot(t,label = 't')
axs[19].plot(t_cds)
axs[20].plot(u,label = 'u')
axs[20].plot(u_cds)
